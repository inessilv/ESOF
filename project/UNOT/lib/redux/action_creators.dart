...
ThunkAction<AppState> getUserExams(Completer<Null> action,
  ...
  return (Store<AppState> store) async {
    try {
      ...
      // Notifications Extension --> New database with id
      final ExamsForNotificationsExtension db2 = ExamsForNotificationsExtension();
      


      //Create notifications for exams volta
      final Notification notification = Notification();
      


      List<Event> events = await getEvents(store);

      EventsDatabase eventsDb = EventsDatabase();
      ...
      if (userPersistentInfo.item1 != '' && userPersistentInfo.item2 != '') {
        ...
        db2.saveNewExams(exams);
        notification.createNotificationsForExams(exams);


         eventsDb.saveNewEvents(events);
        notification.createNotificationsForEvents(events);
...
ThunkAction<AppState> getUserSchedule(
  ...
  return (Store<AppState> store) async {
    try {
      ...
      final List<LectureForNotification> lecturesNotificationsExtension =
          await getLecturesWithScraperNotificationsExtension(store);
      ...
      final AppLecturesForNotificationsDatabase db2 = AppLecturesForNotificationsDatabase();
      ...
      if (userPersistentInfo.item1 != '' && userPersistentInfo.item2 != '') {
        ...
        notification.createNotificationsForClasses(lecturesNotificationsExtension);
        db2.saveNewLectures(lecturesNotificationsExtension);
      }
...
//Used by notifications extension
Future<List<LectureForNotification>> getLecturesWithScraperNotificationsExtension(
    Store<AppState> store) => (getLecturesNotificationsExtension(store));

// This functions use the scrapper lib
Future<List<LectureForNotification>> getLecturesNotificationsExtension(Store<AppState> store) {
  Logger().e('CALLING BASIC');
  return ScheduleFetcherHtml().getLecturesForNotifications(store);
}
