...
class ExamsForNotificationsExtension extends AppDatabase {
  ExamsForNotificationsExtension()
      : super('examsForNotifications.db', [
          '''CREATE TABLE examsForNotifications(examId INTEGER PRIMARY KEY, subject TEXT, begin TEXT, end TEXT,
  ...
  Future<List<Exam>> exams() async {
    ...
    final List<Map<String, dynamic>> maps = await db.query('examsForNotifications');
  ...
  Future <List<Map<String, dynamic>>> getExamsForNotifications() async {
    // Get a reference to the database
    final Database db = await this.getDatabase();
  ...
  Future<void> _insertExams(List<Exam> exams) async {
    for (Exam exam in exams) {
      await insertInDatabase(
        'examsForNotifications',
        exam.toMapNotification(),
  ...
  Future<void> deleteExams() async {
    ...
    await db.delete('examsForNotifications');
  }
}
